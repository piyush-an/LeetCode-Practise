...
select average_sessions_per_user = CASE 
WHEN COUNT(DISTINCT [user_id]) = 0 THEN '0.00'
ELSE
CAST(CAST(COUNT(DISTINCT [session_id])  AS DECIMAL(5,2))/ COUNT (DISTINCT [user_id]) as DECIMAL (5,2))
END
from Activity
--  group by activity_date having 
where
 activity_date BETWEEN DATEADD
(day, -29, '2019-07-27') and '2019-07-27'
-- END